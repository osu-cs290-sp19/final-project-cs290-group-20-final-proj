  ...
  <div class="table">
    <div class="table-time">
      <div class="table-time-blank">
        <select v-model="current" title="Switch to a specific week">
          <option v-for="i in 11" :value="i">Week{{i}} {{i === week ? '(This week)' : ''}}</option>
        </select>
      </div>
      <div class="table-time-evening"><br>8:00</div>
      <div class="table-time-evening"><br>9:00</div>
      <div class="table-time-evening"><br>10:00</div>
      <div class="table-time-evening"><br>11:00</div>
      <div class="table-time-evening"><br>12:00</div>
      <div class="table-time-evening"><br>13:00</div>
      <div class="table-time-evening"><br>14:00</div>
      <div class="table-time-evening"><br>15:00</div>
      <div class="table-time-evening"><br>16:00</div>
    </div>
...
<script>
  ...
  export default {
    ...
    computed: {
      renderDayList() {
        const week_arr = ['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday'];
        const start = new Date(this.start + 'T00:00:00').getTime();
        const length = 5 * (this.current - 1);
      ...
      renderCourseList() {
        ...
        let courseList = this.course.map(item => {
          ...
          if (item.time.start <= this.current && this.current <= item.time.end) {
            valid = true;
            if (item.time.odd === 'odd' && this.current % 2 === 0) valid = false;
            else if (item.time.odd === 'even' && this.current % 2 === 1) valid = false;
          }
